...
- ⛔ **WORKER & CUSTOMER PORTALS LOCKED (owner order, 2026-07-20):** `staff.jayabina.com` (`worker/index.html`) and `akaun.jayabina.com` (`customer/index.html`) must NEVER have their design/UI modified or overwritten without an explicit owner instruction in the current session. These files were rolled back to commit `db18695` (Jul 19) which contains the modern/professional design with desktop sidebar, mobile burger menu drawer, bottom nav bar, stat cards, dark/light theme, and admin-matching green theme `#14532d`. The GrapesJS `protectReason()` guard must also cover `worker/` and `customer/` paths.
